...
\(36\) affine terminal quaternes.  A broader order-four audit shows that
terminal \(24\) is not unique: it occurs in \(236\) square-mask pairs.  Inside
that landscape, the robust object is a \(144\)-pair exact \(V_4\) subclass
characterized by global \(F_2^4\)-affineness; selected-affine near misses are
separated in the atlas by a quotient-shadow obstruction.  Thus \(24\) is not a
universal invariant of magic squares.  It is a structured endpoint in a
...
Section~\ref{sec:polytope} records the bounded-polytope guardrail, and
...
\[
\begin{array}{rcl}
236 & & \text{terminal-24 essential square-mask pairs}\\
 & \supset & 176 \quad \text{main selected-mask-affine / inside-out class}\\
 & & \quad = 144 \quad \text{exact canonical }V_4
      \text{ and globally affine cell-value class}\\
 & & \qquad\quad \supset 8 \quad \text{\Durer/Sagrada quotient cell}\\
 & & \quad + 32 \quad \text{structured non-exact extras}\\
 & \supset & 60 \quad \text{outside the main signature.}
\end{array}
\]
\end{minipage}}
\caption{Order-four terminal-24 containment diagram.  The robust class is the
\(144\)-pair globally affine exact-\(V_4\) subclass; the historical
