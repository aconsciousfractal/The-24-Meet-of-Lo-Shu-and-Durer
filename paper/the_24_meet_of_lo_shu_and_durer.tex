...
\(\F_2^4\) and \(60\) are non-affine.  With respect to the terminal quotient
direction \(\langle u\rangle\), the \(60\) non-affine records split into the
count vector
\[
  60=32+12+16.
\]
Here \(CS_1=32\) counts mixed fold-pair orbits with cell-xor \(u\),
\(CS_2=12\) counts pure non-affine records with non-full quotient direction,
and \(CS_3=16\) counts pure non-affine records with full quotient direction.
This is a classification of records relative to \(\langle u\rangle\), not a
claim about the cardinality of a quotient set.
\end{certprop}
...
Use the same binary cell convention as above: write
\[
r=2r_1+r_0,\qquad c=2c_1+c_0,\qquad x=(r_1,r_0,c_1,c_0).
\]
If
\[
